/* HAVANA NICE — ADMIN STORAGE SUMMARY V2
   Visual Supabase Storage usage for the Free Plan target.
   Uses the authenticated admin session and does not poll.
*/
(function(){
  'use strict';
  ...
  function style(){
    ...
    s.textContent=`
      ...
      .hn-storage-fill{height:100%;width:0;background:#82be91;transition:width .35s ease}.hn-storage-fill.warn{background:#e5bd62}.hn-storage-fill.danger{background:#c66b5f}
      ...
      .hn-storage-details{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:14px}.hn-storage-detail{border:1px solid rgba(130,190,145,.14);padding:10px 9px;background:rgba(0,0,0,.12)}
      ...
      @media(max-width:460px){.hn-storage-head{align-items:flex-start}.hn-storage-used{font-size:20px}.hn-storage-details{grid-template-columns:1fr 1fr}}
  ...
  async function refresh(){
    ...
    try{
      ...
      if(!sessionResult.data?.session){setTimeout(refresh,500);return}
      ...
      $('hnStorageUsed').textContent=fmtMB(bytes)+' MB';$('hnStorageMB').textContent=fmtMB(bytes)+' MB';$('hnStorageGB').textContent=fmtGB(bytes)+' GB';
      $('hnStoragePercent').textContent=pct<0.01?'< 0.01% utilizado':pct.toFixed(2)+'% utilizado';$('hnStorageFree').textContent=fmtMB(free)+' MB disponibles';$('hnStorageLimit').textContent='Límite '+fmtGB(limit)+' GB';
      const fill=$('hnStorageFill'),track=fill?.parentElement;if(fill)fill.style.width=Math.max(bytes?0.35:0,pct)+'%';if(fill){fill.classList.toggle('warn',pct>=70&&pct<90);fill.classList.toggle('danger',pct>=90)}if(track)track.setAttribute('aria-valuenow',pct.toFixed(2));
