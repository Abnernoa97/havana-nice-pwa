...
const ALERT_DELETE='./admin-access-alert-delete-v1.js?v=d6133269c41e4bf36bebcebb82b86a73c01c14c1';
async function normalizeAdmin(response){if(!response||!response.ok)return response;const text=await response.text();let fixed=text;if(!fixed.includes('admin-family-v1.js'))fixed=fixed.replace('</body>',`<script src="${FAMILY_ADMIN}"></script></body>`);if(!fixed.includes('admin-access-alert-delete-v1.js'))fixed=fixed.replace('</body>',`<script src="${ALERT_DELETE}"></script></body>`);const h=new Headers(response.headers);h.delete('content-encoding');h.delete('content-length');h.set('content-type','text/html; charset=utf-8');return new Response(fixed,{status:response.status,statusText:response.statusText,headers:h})}
...
self.addEventListener('fetch',e=>{
  ...
  if(url.pathname.endsWith('/admin-ui-v1.js')||url.pathname.endsWith('/admin-family-v1.js')||url.pathname.endsWith('/admin-storage-summary.js')||url.pathname.endsWith('/admin-access-alert-delete-v1.js')){e.respondWith(fetch(e.request,{cache:'no-store'}).catch(()=>caches.match(e.request)));return}
