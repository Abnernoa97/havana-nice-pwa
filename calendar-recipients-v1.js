/* HAVANA NICE — CALENDAR RECIPIENTS V3 */
(function(){
  'use strict';
  ...
  async function initMusician(){
    ...
    function renderFiltered(rows){const list=document.getElementById('calendarList');if(!list)return;if(rows===null)return;const byId=new Set(rows.map(x=>x.id));list.querySelectorAll('.calendar-event-card').forEach(card=>{card.style.display=byId.has(card.dataset.eventId)?'':'none'});const empty=list.querySelector('.calendar-empty');if(empty)empty.style.display=rows.length?'none':'';list.style.setProperty('visibility','visible','important');}
