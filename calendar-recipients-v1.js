...
(function(){
  ...
  async function initAdmin(){
    let ready=false, active=[], selected=new Set(), currentEventId=null;
    ...
    async function loadActive(){const{data,error}=await s.rpc('admin_list_musicians');if(error){list.innerHTML='<div class="cr-empty">No se pudieron cargar los músicos.</div>';return}active=(data||[]).filter(x=>x.active);render();}
    ...
    document.addEventListener('click',e=>{const b=e.target.closest('.cedit');if(!b)return;setTimeout(()=>selectForEvent(b.dataset.id),0);},true);
  ...
  async function initMusician(){
    ...
    let applying=false;
    async function filter(events){const id=profile();if(!id)return[];const{data,error}=await s.rpc('get_calendar_events_for_profile',{p_profile_id:id});if(error){console.error('Calendar recipients error',error);return[];}return data||[];}
    ...
    async function apply(){if(applying)return;applying=true;try{const rows=await filter();renderFiltered(rows);}finally{applying=false;}}
    ...
    let channel;
    try{channel=s.channel('calendar-recipient-sync').on('postgres_changes',{event:'*',schema:'public',table:'calendar_event_recipients'},()=>apply()).subscribe();}catch(e){console.warn('calendar recipient realtime',e)}
