/* HAVANA NICE — CALENDAR RECIPIENTS V5
   Admin-only recipient editor. Musician calendar visibility is handled directly
   by get_calendar_events_for_profile() in calendar-v1.js.
*/
(function(){
  'use strict';
  ...
  function sb(){
    ...
    if(!clientPromise)clientPromise=import('https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm').then(m=>m.createClient(URL,KEY));
  ...
  async function initAdmin(){
    ...
    async function selectForEvent(id){currentEventId=id;selected=new Set((await getRecipients(id)).map(x=>String(x.profile_id)));render()}
