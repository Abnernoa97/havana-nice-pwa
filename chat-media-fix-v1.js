/* HAVANA NICE — CHAT MEDIA DISPLAY + VIDEO OPTIMIZATION V2 */
(function(){
  'use strict';
  ...
  async function optimizeVideo(file,meta){
    ...
    try{
      ...
      const srcW=video.videoWidth||meta.width||TARGET_W;
      const srcH=video.videoHeight||meta.height||TARGET_H;
      ...
      try{sourceStream=typeof video.captureStream==='function'?video.captureStream():null;}catch(_){sourceStream=null;}
      ...
      const result=await new Promise((resolve,reject)=>{
        ...
        recorder.onstop=()=>{
          ...
          if(!blob.size)reject(new Error('El video comprimido quedó vacío.'));
          else resolve(blob);
        };
      ...
      console.warn('HAVANA NICE video optimization skipped:',error);
      return file;
    }finally{
      try{video.pause();}catch(_){ }
      if(video.src)URL.revokeObjectURL(sourceUrl);
    }
  }

  function createOptimizingNotice(input,count){
    const id='hn-video-optimizing-notice';
    document.getElementById(id)?.remove();
    const el=document.createElement('div');
    el.id=id;
    el.textContent=`OPTIMIZANDO ${count===1?'VIDEO':'VIDEOS'}…`;
  ...
  async function interceptMediaInput(event){
    ...
    if(!input.classList.contains('hn-chat-media-input'))return;
    if(event.__hnOptimizedChange)return;
    ...
    try{
      ...
      for(let i=0;i<files.length;i++){
        ...
        optimized.push(await optimizeVideo(file,meta));
      }
