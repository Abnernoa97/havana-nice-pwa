/* HAVANA NICE — CHAT MEDIA V11
   iPhone-first media layer.
   - History uses lightweight poster cards only.
   - Full video loads only in fullscreen.
   - Large iPhone MOV files are compressed with hardware WebCodecs before Storage upload.
   - No server-side transcode bridge; upload preparation lives in one media layer.
*/
(function(){
  'use strict';
  if(window.__hnChatMediaV11)return;
  window.__hnChatMediaV11=true;

  const INPUT='.hn-chat-media-input';
  const BUCKET='chat-media';
  const STYLE='hn-chat-media-v11-style';
  const VIEWER='hnChatMediaV11';
  const HISTORY='hnChatMediaV11';
  const MAX_EDGE=640;
  const POSTER_QUALITY=.76;
  const POSTER_TIMEOUT=9000;
  const COMPRESS_MIN_BYTES=6*1024*1024;
  const COMPRESS_ACCEPT_RATIO=.88;
  const MEDIABUNNY_URL='https://esm.sh/mediabunny@1.52.0?bundle&target=es2022';
  ...
  const isLargeIOSVideo=file=>isIOS()&&isVideo(file)&&file.size>=COMPRESS_MIN_BYTES&&(/quicktime/i.test(file.type||'')||/\.mov$/i.test(file.name||''));

  function loadMediaLib(){
    if(mediaLibPromise)return mediaLibPromise;
    mediaLibPromise=import(MEDIABUNNY_URL).catch(error=>{
      mediaLibPromise=null;
      console.warn('HAVANA NICE browser video compressor unavailable:',error);
      return null;
    });
  ...
  async function compressForUpload(file){
    if(!isLargeIOSVideo(file))return file;
    if(typeof window.VideoEncoder!=='function'||typeof window.VideoDecoder!=='function')return file;
    let input=null;
    try{
      const lib=await loadMediaLib();
      if(!lib)return file;
      const {ALL_FORMATS,BlobSource,BufferTarget,Conversion,Input,Mp4OutputFormat,Output,Quality}=lib;
      ...
      const conversion=await Conversion.init({
        ...
        video:async track=>({
          ...
          quality:new Quality({bitrate:1400000})
        }),
        tags:{}
      });
      ...
      if(!buffer?.byteLength||buffer.byteLength>=file.size*COMPRESS_ACCEPT_RATIO)return file;
      const name=(file.name||'video').replace(/\.[^.]+$/,'')+'-hn.mp4';
      const compressed=new File([buffer],name,{type:'video/mp4',lastModified:Date.now()});
      console.info('HAVANA NICE video prepared for upload',{
        originalBytes:file.size,
        uploadBytes:compressed.size,
        ratio:Number((compressed.size/file.size).toFixed(3))
      });
      return compressed;
    }catch(error){
      console.warn('HAVANA NICE browser video compression skipped:',error);
      return file;
    }finally{
  ...
  function makeEntry(file,g){
    const entry={file,blob:null,url:'',posterPromise:null,uploadPromise:null};
    ...
    entry.uploadPromise=entry.posterPromise.catch(()=>null).then(()=>compressForUpload(file));
    return entry;
  }
  ...
  async function prepareUpload(file){
    ...
    if(entry?.uploadPromise)return entry.uploadPromise;
  ...
  function paintPending(){
    ...
    [screen.querySelector('.hn-chat-media-pending'),screen.querySelector('.hn-chat-row[data-chat-key="outgoing"]')].filter(Boolean).forEach(container=>{
      [...container.querySelectorAll('.hn-chat-pending-item,.hn-chat-media-item')].forEach((item,i)=>{
        ...
        let img=item.querySelector('.hn-v11-local-poster');
        if(!img){img=document.createElement('img');img.className='hn-v11-local-poster';img.alt='Vista previa del video';item.appendChild(img)}
  ...
  function installUploadAdapter(){
    ...
    if(storage.__hnChatVideoUploadV11){patchedStorage=storage;return}
    try{
      const originalFrom=storage.from.bind(storage);
      storage.from=function(bucket){
        const api=originalFrom(bucket);
        if(bucket!==BUCKET||api.__hnVideoUploadWrapped)return api;
        const originalUpload=api.upload.bind(api);
        api.upload=async function(path,body,options){
          if(body instanceof File&&isVideo(body)){
            const prepared=await prepareUpload(body);
            if(prepared&&prepared!==body){
              return originalUpload(path,prepared,{...(options||{}),contentType:prepared.type||'video/mp4'});
            }
          }
          return originalUpload(path,body,options);
        };
        api.__hnVideoUploadWrapped=true;
        return api;
      };
      storage.__hnChatVideoUploadV11=true;
  ...
  function ensureViewer(){
    ...
    overlay.innerHTML='<button class="hn-media-v11-close" type="button" aria-label="Cerrar">×</button><img class="hn-media-v11-image" alt=""><img class="hn-media-v11-poster" alt=""><video class="hn-media-v11-video" playsinline controls preload="none"></video><div class="hn-media-v11-spinner"></div><button class="hn-media-v11-play" type="button">▶</button><div class="hn-media-v11-error">NO SE PUDO CARGAR EL VIDEO</div>';
    document.body.appendChild(overlay);
    image=overlay.querySelector('.hn-media-v11-image');poster=overlay.querySelector('.hn-media-v11-poster');video=overlay.querySelector('.hn-media-v11-video');spinner=overlay.querySelector('.hn-media-v11-spinner');play=overlay.querySelector('.hn-media-v11-play');errorBox=overlay.querySelector('.hn-media-v11-error');
    overlay.querySelector('.hn-media-v11-close').addEventListener('click',closeFromUser);
  ...
  function installStyles(){
    ...
    const s=document.createElement('style');s.id=STYLE;s.textContent=`
      ...
      #hn-chat-screen .hn-v11-local-poster{position:absolute!important;inset:0!important;width:100%!important;height:100%!important;object-fit:cover!important;z-index:6!important;background:#090909!important}
      ...
      #${VIEWER} .hn-media-v11-image,#${VIEWER} .hn-media-v11-poster,#${VIEWER} .hn-media-v11-video{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);width:auto;height:auto;max-width:100vw;max-height:100dvh;object-fit:contain;background:#000}
      #${VIEWER} .hn-media-v11-video{z-index:2;opacity:0;transition:opacity .12s linear}#${VIEWER} .hn-media-v11-poster{z-index:3;opacity:1;transition:opacity .12s linear}#${VIEWER}.is-playing .hn-media-v11-video{opacity:1}#${VIEWER}.is-playing .hn-media-v11-poster{opacity:0;pointer-events:none}#${VIEWER} .hn-media-v11-image{z-index:3}
      #${VIEWER} .hn-media-v11-close{position:absolute;top:max(14px,calc(env(safe-area-inset-top) + 8px));right:max(14px,calc(env(safe-area-inset-right) + 8px));z-index:20;width:46px;height:46px;border-radius:50%;border:1px solid rgba(229,189,98,.78);background:rgba(0,0,0,.58);color:#fff1a8;font-size:28px}
      #${VIEWER} .hn-media-v11-spinner{position:absolute;left:50%;top:50%;z-index:10;width:38px;height:38px;margin:-19px;border:3px solid rgba(255,255,255,.20);border-top-color:#fff;border-radius:50%;animation:hnMediaV11Spin .8s linear infinite}
      #${VIEWER} .hn-media-v11-play{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);z-index:11;width:62px;height:62px;border-radius:50%;border:1px solid rgba(255,255,255,.65);background:rgba(0,0,0,.55);color:#fff;font-size:25px;padding-left:5px}
      #${VIEWER} .hn-media-v11-error{position:absolute;left:50%;bottom:max(42px,calc(env(safe-area-inset-bottom) + 28px));transform:translateX(-50%);z-index:12;color:#fff;font:500 10px/1.3 Arial,sans-serif;letter-spacing:.12em;white-space:nowrap}@keyframes hnMediaV11Spin{to{transform:rotate(360deg)}}
  ...
  function install(){
    ...
    window.hnChatMediaV10={prepare,ensurePoster,persistPoster,prepareUpload,posterUrl,paintPending,clearSelection,openPhoto,openVideo};
