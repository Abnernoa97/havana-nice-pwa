/* HAVANA NICE — CHAT DE INFORMACIÓN / REAL TIME V8 */
(() => {
  'use strict';
  ...
  function render(forceBottom=false){
    ...
    [...listEl.children].forEach(node=>{if(!keep.has(node))node.remove();});
    ...
    wireMessageInteractions();if(forceBottom||nearBottom)scrollToLatest();
  }
  ...
  async function sendMessage(e){e.preventDefault();if(mediaRecorder&&mediaRecorder.state==='recording')return;const sb=client(),p=currentProfile();if(!sb||!p?.id)return;sendEl.disabled=true;try{if(pendingVoice){const voice=pendingVoice;pendingVoice=null;await uploadVoiceNote(voice.blob,voice.duration);inputEl.value='';inputEl.style.height='auto';closeReply();resetRecordingUI(false);return;}if(pendingMedia.length){await uploadMedia();inputEl.value='';inputEl.style.height='auto';closeReply();return;}const text=String(inputEl.value||'').trim();if(!text)return;const payload={profile_id:p.id,sender_name:p.username||'MIEMBRO',message:text.slice(0,MAX_LENGTH),message_type:'text'};if(replyTarget?.id)payload.reply_to_message_id=replyTarget.id;const ins=await sb.from(CHAT_TABLE).insert(payload);if(ins.error)throw ins.error;inputEl.value='';inputEl.style.height='auto';closeReply();}catch(error){console.error('HAVANA NICE chat send failed:',error);alert(error?.message||'No se pudo enviar.');}finally{updateSendState();inputEl?.focus();}}
