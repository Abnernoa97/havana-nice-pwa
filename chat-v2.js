/* HAVANA NICE — CHAT DE INFORMACIÓN / REAL TIME V9 — LOCAL HISTORY / INCREMENTAL SYNC */
(() => {
  'use strict';
  ...
  const MESSAGE_FIELDS='id,profile_id,sender_name,message,created_at,message_type,audio_url,audio_duration,media_urls,media_types,reply_to_message_id';
  ...
  // A disposable, per-member copy. Supabase remains authoritative.
  ...
  function persistHistory(){
    ...
    void historyOperation('readwrite',id,{savedAt:Date.now(),rows:messages.slice(-200)});
  }
  function ensureChatIdentity(){
    const id=String(currentProfile()?.id||'');
    if(id===activeProfileId)return id;
    activeProfileId=id;historyRestoredFor=null;messages=[];syncedAt=0;
  ...
  async function restoreHistory(){
    ...
    messages=saved.rows.filter(row=>row&&row.id&&row.created_at).slice(-200);
    render(true);updateUnread();
  }
  function noteLiveChange(row,deleted=false){
    syncRevision++;
    liveChanges.set(String(row.id),{revision:syncRevision,row,deleted});
  }
  ...
  // Only small photos are cached, never entire videos or audio.
  // Direct browser requests preserve the iOS Service Worker exception.
  const PHOTO_CACHE_PREFIX='hn-chat-photos-v1-';
  let photoWrite=Promise.resolve();
  function releaseLocalImages(root){
    root.querySelectorAll('img[data-hn-local-url]').forEach(img=>URL.revokeObjectURL(img.dataset.hnLocalUrl));
  }
  async function cachedPhoto(img,url){
    ...
    try{
      ...
      if(hit&&Date.now()-Number(hit.headers.get('x-hn-saved-at'))<HISTORY_TTL){
        ...
        const local=URL.createObjectURL(blob);img.dataset.hnLocalUrl=local;img.src=local;return;
      }
    }catch(_){}
  }
    ...
    photoWrite=photoWrite.then(async()=>{
      ...
      try{
        ...
        try{
          ...
          // 24 entries, each <=1 MB. Eviction affects only local copies.
          for(const key of keys.slice(0,Math.max(0,keys.length-23)))await cache.delete(key);
  ...
  function wirePhotoCache(){
    listEl.querySelectorAll('.hn-chat-photo-button img').forEach(img=>{
      ...
      // Give local storage a short head start, then use the normal network path.
      ...
      if(img.complete&&img.naturalWidth)warmPhoto(url);
      else img.addEventListener('load',()=>warmPhoto(url),{once:true});
    });
  }
  function showOutgoing(){
    ...
    if(pendingMedia.length){
      ...
      pendingMedia.forEach(file=>{
        const item=document.createElement('div');item.className='hn-chat-media-item';
        const media=document.createElement(file.type.startsWith('image/')?'img':'video');
        const url=URL.createObjectURL(file);outgoingUrls.push(url);media.src=url;
        if(media.tagName==='VIDEO'){media.muted=true;media.playsInline=true;media.preload='metadata';}
        else media.alt='Foto pendiente de envío';
        item.appendChild(media);grid.appendChild(item);
      });bubble.appendChild(grid);
    }else{
      const text=document.createElement('div');text.className='hn-chat-text';
      text.textContent=pendingVoice?'NOTA DE VOZ':String(inputEl.value||'').trim();bubble.appendChild(text);
    }
    ...
    outgoing.appendChild(bubble);listEl.appendChild(outgoing);scrollToLatest();
  }
  function clearOutgoing(){
    outgoing?.remove();outgoing=null;
    outgoingUrls.forEach(url=>URL.revokeObjectURL(url));outgoingUrls=[];
  }
  ...
  function getReplyTarget(message) {
    if(message?.reply_to_message_id) return messages.find(m => String(m.id)===String(message.reply_to_message_id)) || null;
    return null;
  }
  ...
  function scrollToLatest(){
    ...
    listEl.querySelectorAll('img,video').forEach(media=>{
      ...
      if(media.dataset.hnScrollBound==='1')return;
      media.dataset.hnScrollBound='1';media.addEventListener(event,move,{once:true});
    });
  }
  ...
  function render(forceBottom=false){
    ...
    messages.forEach((message,index)=>{
      ...
      const signature=JSON.stringify([message.id,message.profile_id,message.sender_name,message.message,message.created_at,message.message_type,message.audio_url,message.audio_duration,message.media_urls,message.media_types,message.reply_to_message_id,replyPreview(getReplyTarget(message)),getReplyTarget(message)?.sender_name,currentProfile()?.id]);
      const previous=existing.get(key);
      if(previous?.dataset.chatSignature===signature){desired.push(previous);return;}
    ...
    // Keep unchanged nodes connected: Safari retains decoded images and media state.
    if(outgoing)desired.push(outgoing);
    const keep=new Set(desired);
    [...listEl.children].forEach(node=>{if(!keep.has(node)){releaseLocalImages(node);node.remove();}});
    let cursor=listEl.firstChild;
    desired.forEach(node=>{if(node===cursor)cursor=cursor.nextSibling;else listEl.insertBefore(node,cursor);});
  ...
  function wireMessageInteractions(){
    ...
    listEl.querySelectorAll('.hn-chat-photo-button').forEach(btn=>{if(btn.dataset.hnInteractions==='1')return;btn.dataset.hnInteractions='1';btn.addEventListener('click',e=>{e.stopPropagation();if(window.hnOpenChatMedia)window.hnOpenChatMedia(btn.dataset.photoUrl,'image');});});
  ...
  async function uploadMedia(){
    const sb=client(),p=currentProfile(),files=pendingMedia.slice();
    if(!sb||!p?.id||!files.length)return;
    const urls=new Array(files.length),types=files.map(file=>file.type||''),paths=[];
    let next=0,failed=null,committed=false;
    const worker=async()=>{
      while(next<files.length&&!failed){
        const index=next++,file=files[index];
        const ext=(file.name.split('.').pop()||'bin').toLowerCase();
        const path=`${p.id}/${Date.now()}-${crypto.randomUUID()}.${ext}`;
        try{
          const up=await sb.storage.from(MEDIA_BUCKET).upload(path,file,{contentType:file.type||'application/octet-stream',upsert:false});
          if(up.error)throw up.error;
          paths.push(path);
          urls[index]=sb.storage.from(MEDIA_BUCKET).getPublicUrl(path).data.publicUrl;
        }catch(error){failed=error;}
      }
    };
    try{
      // Two small photos at once; videos retain the existing sequential path.
      const concurrency=files.every(file=>file.type.startsWith('image/'))?Math.min(2,files.length):1;
      await Promise.all(Array.from({length:concurrency},worker));
      if(failed)throw failed;
      const ins=await sb.from(CHAT_TABLE).insert({profile_id:p.id,sender_name:p.username||'MIEMBRO',message:'',message_type:'media',media_urls:urls,media_types:types}).select(MESSAGE_FIELDS).single();
      if(ins.error)throw ins.error;
      committed=true;
      if(String(p.id)!==activeProfileId)return;
      clearOutgoing();mergeConfirmed(ins.data);
      pendingMedia=[];clearPendingPreviewUrls();renderPendingMedia();
    }catch(error){
      if(!committed)await removeUploadedObjects(sb,MEDIA_BUCKET,paths);
      throw error;
    }
  }
  ...
  async function loadMessages(){
    const id=ensureChatIdentity(),sb=client();if(!id||!sb)return;
    if(loadFlight)return loadFlight;
    const revision=syncRevision;
    const task=(async()=>{
      try{
        const {data,error}=await sb.from(CHAT_TABLE).select(MESSAGE_FIELDS).order('created_at',{ascending:false}).limit(200);
        ...
        // A slow SELECT must not erase an INSERT/DELETE received while it was in flight.
        for(const [key,change] of liveChanges){
          if(change.revision<=revision)continue;
          if(change.deleted)rows.delete(key);else rows.set(key,change.row);
        }
        messages=[...rows.values()].sort((a,b)=>new Date(a.created_at)-new Date(b.created_at));
        syncRevision++;syncedAt=Date.now();
        for(const [key,change] of liveChanges)if(change.revision<=revision)liveChanges.delete(key);
        persistHistory();render(!!chatScreen?.classList.contains('is-active'));updateUnread();
      }catch(error){console.warn('HAVANA NICE chat reconcile failed:',error);}
    })();
    loadFlight=task;
    try{await task;}finally{if(loadFlight===task)loadFlight=null;}
  }
