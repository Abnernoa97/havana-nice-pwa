/* HAVANA NICE — iOS FAMILY MEDIA COMPATIBILITY V3
...
async function load(img){
  ...
  if(busy.has(url)){
    try{
      const blobUrl=await busy.get(url);
      if(blobUrl&&img.isConnected){img.src=blobUrl;img.style.display='block'}
    }catch(_){}
    return;
  }
  const p=(async()=>{
    try{
      const r=await fetch(url,{method:'GET',mode:'cors',cache:'no-store',headers:{apikey:KEY}});
      if(!r.ok)throw Error(String(r.status));
      const blob=await r.blob();
      if(!blob.size)throw Error('empty');
      const blobUrl=URL.createObjectURL(blob);
      loaded.set(url,blobUrl);
      return blobUrl;
    }catch(e){
      console.warn('[HN iOS family media]',e);
      return '';
    }
  })();
  busy.set(url,p);
...
function scan(){
  document.querySelectorAll('.family-member-avatar img').forEach(img=>{
