...
function clearMedia(node){if(!node)return;node.querySelectorAll('.hn-family-media-isolation').forEach(x=>x.remove());node.style.removeProperty('background-image')}
function paint(node,path,key,updated,round=false){clearMedia(node);if(!path)return;const img=document.createElement('img');img.className='hn-family-media-isolation';img.alt='';img.draggable=false;img.decoding='sync';img.loading='eager';img.src=url(path,key,updated);img.style.cssText='position:absolute;inset:0;width:100%;height:100%;display:block;object-fit:cover;border:0;margin:0;padding:0;pointer-events:none;'+(round?'border-radius:50%;':'');node.appendChild(img)}
