...
function paint(node,path,key,updated,round=false){if(!node)return;const desired=desiredSrc(path,key,updated);const existing=node.querySelector('.hn-family-media-isolation');if(desired&&existing?.getAttribute('src')===desired){node.style.removeProperty('background-image');return}if(!desired&&!existing&&!node.style.backgroundImage)return;node.querySelectorAll('.hn-family-media-isolation').forEach(x=>x.remove());node.style.removeProperty('background-image');if(!desired)return;node.style.position='relative';const img=document.createElement('img');img.className='hn-family-media-isolation';img.alt='';img.draggable=false;img.decoding='sync';img.loading='eager';img.src=desired;img.style.cssText='position:absolute;inset:0;width:100%;height:100%;display:block;object-fit:cover;border:0;margin:0;padding:0;pointer-events:none;'+(round?'border-radius:50%;':'');node.appendChild(img)}
