/* HAVANA NICE — FAMILY PROFILE SHELL V2
...
(() => {
  ...
  const publicUrl = (path) => {
    ...
    return supabase?.storage.from('family-media').getPublicUrl(path)?.data?.publicUrl || `${SUPABASE_URL}/storage/v1/object/public/family-media/${path}`;
  };

  function ensureStyles() {
    if (document.getElementById('family-profile-shell-v2-style')) return;
    const style = document.createElement('style');
    style.id = 'family-profile-shell-v2-style';
    style.textContent = `
      ...
      .hn-family-profile-cover:after{content:'';position:absolute;inset:0;background:linear-gradient(180deg,rgba(0,0,0,.05),rgba(0,0,0,.62))}
      .hn-family-profile-avatar{position:absolute;left:24px;bottom:-56px;width:112px;height:112px;border-radius:50%;box-sizing:border-box;border:2px solid #d9b45f;background:#0a110d center/cover no-repeat;box-shadow:0 8px 28px rgba(0,0,0,.42),0 0 0 5px rgba(0,0,0,.32);z-index:2}
  ...
  async function loadProfiles() {
    if (!window.hnSupabase) return;
    supabase = window.hnSupabase;
  ...
  function render(index) {
    const p = profiles[index] || { name: ['FER & NOA','ORLYS SHOW','JALI','RAFA','ANDY REY'][index] || '', role: index === 0 ? 'HAVANA NICE' : 'MÚSICO', bio: '', avatar_path: null, cover_path: null, member_key: FAMILY_KEYS[index] };
    currentIndex = index;
    ...
    avatar.style.backgroundImage = p.avatar_path ? `url("${publicUrl(p.avatar_path)}")` : '';
  ...
  function refreshVisibleProfile() {
    if (profileScreen?.classList.contains('is-active')) render(currentIndex);
  }
  ...
  async function compressImage(file, mediaType) {
    ...
    bitmap.close?.();
    const blob = await new Promise((resolve, reject) => canvas.toBlob(b => b ? resolve(b) : reject(new Error('No fue posible preparar la imagen')), 'image/webp', quality));
  ...
  async function handleUpload(mediaType, originalFile) {
    ...
    try {
      ...
      const response = await fetch(MEDIA_ENDPOINT, { method: 'POST', body: form });
      ...
      profileScreen.querySelector(`input[data-media="${mediaType}"]`).value = '';
    }
  }
  ...
  function init() {
    build();
    wireFamily();
    setTimeout(() => { loadProfiles(); subscribeRealtime(); }, 100);
  }
