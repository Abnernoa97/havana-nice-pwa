/* HAVANA NICE — FAMILIA / CHAT NAVIGATION */
(() => {
  'use strict';
  ...
  function buildFamily() {
    ...
    if (!document.getElementById('hn-family-styles')) {
      ...
      style.textContent = `
        ...
        .family-top-back,.family-back { flex:0 0 auto; width:100%; height:44px; border:1px solid rgba(229,189,98,.35); border-radius:0; background:rgba(0,0,0,.25); color:rgba(244,241,232,.72); font-size:9px; letter-spacing:.2em; text-transform:uppercase; }
        .family-top-back { margin:0 0 8px; }
        .family-back { margin-top:8px; }
      `;
    ...
    familyScreen.innerHTML = `
      <div class="family-inner">
        ...
        <button id="familyTopBackButton" class="family-top-back" type="button">VOLVER</button>
    ...
    familyScreen.querySelector('#familyTopBackButton')?.addEventListener('click', () => closeFamily(true));
  ...
  function init() {
    ...
    module.querySelector('.module-subtitle')?.replaceChildren(document.createTextNode('EQUIPO HAVANA NICE'));
