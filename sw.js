...
const FAMILY_IOS_VERSION='ios-family-20260918-1852';
...
const FRESH_PATHS=new Set([
  ...
  '/family-v1.js','/family-ios-diagnostic-v1.js',
...
async function normalizeMusicianShell(response){
  ...
  if(IS_IOS){
    text=text.replace(/\.\/family-v1\.js\?v=[^"']+/g,'./family-v1.js?v='+FAMILY_IOS_VERSION);
    if(!text.includes('family-ios-diagnostic-v1.js')){
      text=text.replace('</body>','<script src="./family-ios-diagnostic-v1.js?v='+FAMILY_IOS_VERSION+'"></script></body>');
    }
  }
...
async function normalizeAdminShell(response){
  ...
  let text=await response.text();
  text=text.split(SUPABASE_CDN).join(SUPABASE_ESM);
  // Legacy file no longer exists. Do not let installed admin shells request it.
  text=text.replace(/<script[^>]+src=["'][^"']*notifications-admin-fix\.js[^"']*["'][^>]*><\/script>/gi,'');
  // admin-family-v1.js is loaded by admin-ui-v1.js; do not inject a second copy.
  return normalizedHtmlResponse(response,text);
}
...
async function normalizeShell(response,admin){
  return admin?normalizeAdminShell(response):normalizeMusicianShell(response);
}
...
self.addEventListener('fetch',event=>{
  ...
  // Preserve the proven iOS behavior: cross-origin Supabase/media requests bypass SW.
  ...
  if(IS_IOS&&url.pathname.endsWith('/family-v1.js')){
    ...
    event.respondWith(fetch(new Request(fresh.toString(),{
      method:'GET',credentials:'same-origin',cache:'no-store'
    })).catch(()=>networkFirstAsset(event.request)));
    return;
  }
...
self.addEventListener('push',event=>{
  event.waitUntil((async()=>{
    ...
    try{
      if(self.navigator&&self.navigator.setAppBadge)await self.navigator.setAppBadge(Math.min(99,count));
    }catch(_){}
